	...
	return (
		<div className="min-h-screen bg-black text-gray-100 flex font-mono">
			<aside className="w-64 bg-gray-900 border-r border-gray-800 p-6 hidden md:block">
				<Link href="/#home" className="text-green-400 font-bold text-xl mb-8 block hover:underline">
					DedCore Docs
				</Link>
...
function SidebarLink({ href, children }: { href: string; children: React.ReactNode }) {
	...
	return (
		<Link
			href={href}
			className={`block px-3 py-2 rounded transition-colors text-gray-200 hover:text-green-400 hover:bg-gray-800 ${
				isActive ? "bg-gray-800 text-green-400 font-bold" : ""
			}`}
		>
			{children}
		</Link>
